...
###############################################################
# TASK 1: Read the dataset and select the variables to be used for customer segmentation.
###############################################################
...
df_ = pd.read_csv("/Users/crmAnalytics/flo_data_20K.csv")
df = df_.copy()
...
df["recency"] = (analysis_date - df["last_order_date"]).astype('timedelta64[D]') # how many days ago did they last shop
...
# Right-skewed (left-tailed) distribution. (mode < median < mean)
# As skewness is present, log transformation should be applied.

# Applying Log transformation for achieving normal distribution
